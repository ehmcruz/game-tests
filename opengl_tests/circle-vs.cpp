...
int main( int argc, char **argv )
{
	...
	circle_factory.fill_vertex_buffer(50.0f, &(g_vertex_buffer_data->x), &(g_vertex_buffer_data->y), 8);
	...
	for (uint32_t i=0; i<circle_factory.get_n_vertices(); i++) {
		...
		g_vertex_buffer_data[i].offset_x = 300.0f;
		g_vertex_buffer_data[i].offset_y = 300.0f;
