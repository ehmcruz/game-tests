...
static const char * fragment_shader =
	...
	"    float ratio = dist / v_radius;\n"
	"    o_color = vec4(v_color.x * ratio, v_color.y * ratio, v_color.z * ratio, v_color.w);\n"
	"}\n";
...
int main( int argc, char **argv )
{
	...
	glBindAttribLocation( program, attrib_radius, "i_radius" );
	...
	struct gl_vertex_t {
		...
		GLfloat offset_y;
		GLfloat radius;
		GLfloat dummy[7]; // just to keep stride a power-of-two
	};
	...
	glVertexAttribPointer( attrib_radius, 1, GL_FLOAT, GL_FALSE, sizeof(gl_vertex_t), ( void * )(8 * sizeof(float)) );

	uint32_t n_vertices = 6;

	gl_vertex_t *g_vertex_buffer_data = new gl_vertex_t[ n_vertices ];

	float radius = 150.0f;
	float offset_x = 300.0f;
	float offset_y = 300.0f;

	// add vertices clock-wise

	// first triangle
	g_vertex_buffer_data[0] = {0.0f, 1.0f, 0.0f, 1.0f, -radius, -radius, offset_x, offset_y, radius};
	g_vertex_buffer_data[1] = {0.0f, 1.0f, 0.0f, 1.0f,  radius,  radius, offset_x, offset_y, radius};
	g_vertex_buffer_data[2] = {0.0f, 1.0f, 0.0f, 1.0f, -radius,  radius, offset_x, offset_y, radius};

	// second triangle
	g_vertex_buffer_data[3] = {0.0f, 1.0f, 0.0f, 1.0f, -radius, -radius, offset_x, offset_y, radius};
	g_vertex_buffer_data[4] = {0.0f, 1.0f, 0.0f, 1.0f,  radius, -radius, offset_x, offset_y, radius};
	g_vertex_buffer_data[5] = {0.0f, 1.0f, 0.0f, 1.0f,  radius,  radius, offset_x, offset_y, radius};

	//exit(0);

	//std::cout << "check c size " << sizeof(g_vertex_buffer_data) << std::endl;

	glBufferData( GL_ARRAY_BUFFER, sizeof(gl_vertex_t) * n_vertices, g_vertex_buffer_data, GL_DYNAMIC_DRAW );
